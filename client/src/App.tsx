		...
		<>
			...
			>
				<div className="h-16 flex items-center justify-between px-6">
					<a
						...
						href="/"
						aria-label="Resume Improver home"
					>
					...
					{!isMobile && (
						...
						>
							<li>
								<a
									href="#how-it-works"
									className="text-sm text-[#7c8399] no-underline transition-colors duration-200 hover:text-[#edf0f7]"
								>
									How it works
								</a>
							</li>
							...
							<li>
								<a
									href="#upload"
									className="font-sora text-[0.85rem] font-semibold text-white no-underline bg-[#4f8ef7] px-[18px] py-[7px] rounded-lg transition-all duration-200 hover:bg-[#3570d4] hover:-translate-y-px"
					...
					{!isMobile && (
						<div className="flex items-center gap-3">
							{user ? (
								<>
									<span className="text-[0.8rem] text-[#7c8399]">
										Hi, {user.name}
									</span>
									<button
										type="button"
										className="font-sora text-[0.8rem] font-medium text-white bg-transparent rounded-full border border-[#2e3340] px-[14px] py-[6px] cursor-pointer transition-all duration-200 hover:bg-[rgba(79,142,247,0.1)] hover:border-[#4f8ef7] hover:-translate-y-px"
					...
					{isMobile && (
						...
						>
							<span
								className={`block w-4 h-[1.5px] bg-[#7c8399] transition-all duration-200 origin-center ${mobileMenuOpen ? "rotate-45 translate-y-[6.5px]" : ""}`}
							/>
							<span
								className={`block w-4 h-[1.5px] bg-[#7c8399] transition-all duration-200 ${mobileMenuOpen ? "opacity-0" : ""}`}
							/>
							<span
								className={`block w-4 h-[1.5px] bg-[#7c8399] transition-all duration-200 origin-center ${mobileMenuOpen ? "-rotate-45 -translate-y-[6.5px]" : ""}`}
							/>
					...
					<div className="border-t border-[#22262f] bg-[#08090b]/95 px-6 py-4 flex flex-col gap-3">
						<ul className="list-none m-0 p-0 flex flex-col gap-3" role="list">
							...
							<li>
								<a
									href="#upload"
									className="inline-block font-sora text-[0.85rem] font-semibold text-white no-underline bg-[#4f8ef7] px-[18px] py-[7px] rounded-lg transition-all duration-200 hover:bg-[#3570d4]"
									onClick={() => setMobileMenuOpen(false)}
								>
									Try it free
								</a>
						...
						<div className="border-t border-[#22262f] pt-3">
							{user ? (
								<div className="flex items-center justify-between">
									...
									<button
										type="button"
										className="font-sora text-[0.8rem] font-medium text-white bg-transparent rounded-full border border-[#2e3340] px-[14px] py-[6px] cursor-pointer transition-all duration-200 hover:bg-[rgba(79,142,247,0.1)] hover:border-[#4f8ef7]"
										onClick={() => {
											handleLogout();
								...
								<button
									type="button"
									className="w-full font-sora text-[0.8rem] font-medium text-white bg-transparent rounded-full border border-[#2e3340] px-[14px] py-[6px] cursor-pointer transition-all duration-200 hover:bg-[rgba(79,142,247,0.1)] hover:border-[#4f8ef7]"
									onClick={() => {
										setIsLoginMode(true);
										setShowAuthModal(true);
										setMobileMenuOpen(false);
			...
			>
				...
				<div
					...
					style={{
						background:
							"radial-gradient(ellipse, rgba(79,142,247,0.09) 0%, transparent 68%)",
					}}
					aria-hidden="true"
				/>
				...
				>
					<span
						className="w-1.5 h-1.5 bg-[#4f8ef7] rounded-full animate-pulse-dot"
						aria-hidden="true"
					/>
				...
				>
					<span>Scroll</span>
					<span
						className="w-[18px] h-[18px] border-r-[1.5px] border-b-[1.5px] border-[#454d62] animate-arrow-bounce"
			...
			{/* ── Stats strip ── */}
			<div
				className="border-t border-b border-[#22262f] bg-[#0e1014] py-10 px-8"
				role="region"
				...
				<dl className="max-w-[860px] mx-auto grid grid-cols-3 gap-4 text-center">
					{[
						...
						<div key={label} className="py-2">
							<dt className="text-[0.78rem] text-[#454d62] mt-[5px] tracking-[0.04em]">
								{label}
							</dt>
							<dd className="font-sora text-[1.9rem] font-extrabold text-[#edf0f7] tracking-[-0.04em]">
								{value}
								<span className="text-[#4f8ef7]">{suffix}</span>
							</dd>
						</div>
					))}
				</dl>
			...
			>
				<div className="max-w-[1080px] mx-auto">
					<span
						className="inline-block font-sora text-[0.68rem] font-semibold tracking-[0.14em] uppercase text-[#4f8ef7] mb-3.5"
						aria-hidden="true"
					>
						How it works
					</span>
					<h2
						id="steps-heading"
						className="font-sora text-[clamp(1.55rem,3.5vw,2.25rem)] font-bold tracking-[-0.03em] text-[#edf0f7] mb-3.5"
					>
						Three steps to a better resume
					</h2>
					<p className="text-[0.975rem] text-[#7c8399] leading-[1.75] font-light max-w-[480px] mx-auto">
						No account required. No templates to fill in. Just upload and
						download.
					</p>
					...
					>
						{steps.map((step) => (
							...
							>
								...
								<span className="block text-2xl mb-3.5" aria-hidden="true">
									{step.icon}
								</span>
								<h3 className="font-sora text-[0.95rem] font-semibold text-[#edf0f7] mb-[0.45rem]">
									{step.title}
								</h3>
								<p className="text-[0.855rem] text-[#7c8399] leading-[1.65] font-light">
									{step.desc}
								</p>
							</li>
						))}
			...
			>
				<div className="max-w-[1080px] mx-auto px-8 py-[6.5rem] text-center">
					...
					<ul
						className="grid grid-cols-[repeat(auto-fit,minmax(270px,1fr))] gap-4 mt-12 list-none"
						role="list"
					>
						{features.map((feat) => (
							<li
							...
							>
								...
								<h3 className="font-sora text-[0.93rem] font-semibold text-[#edf0f7] mb-[0.45rem]">
									{feat.title}
								</h3>
								<p className="text-[0.845rem] text-[#7c8399] leading-[1.65] font-light">
									{feat.desc}
								</p>
							</li>
						))}
			...
			>
                ...
                ${
						...
						>
							{file ? (
								<>
									...
									<span className="text-[0.88rem] font-medium text-[#34c77b]">
										{file.name}
									</span>
								...
								<>
									...
									<span className="text-[0.9rem] font-medium text-[#edf0f7]">
					...
					<p
						className="mt-4 flex items-center justify-center gap-1.5 text-[0.74rem] text-[#454d62]"
						role="note"
					>
			...
			{/* ── Footer ── */}
			<footer
				className="border-t border-[#22262f] bg-[#0e1014] py-10 px-10"
				role="contentinfo"
			>
				<div className="max-w-[1080px] mx-auto flex items-center justify-between flex-wrap gap-4">
					...
					<ul className="flex gap-7 list-none" role="list">
						<li>
							<a
								href="#how-it-works"
								className="text-[0.78rem] text-[#454d62] no-underline transition-colors duration-200 hover:text-[#7c8399]"
							>
								How it works
							</a>
						</li>
						...
						<div className="flex bg-[#0e1014] rounded-full border border-[#22262f] p-[3px] mx-auto mb-6 w-[156px] justify-center">
							<button
								...
								className={`border-none font-sora text-[0.8rem] font-medium px-[14px] py-[6px] rounded-full cursor-pointer transition-all duration-200 ${
									isLoginMode
										? "bg-[#4f8ef7] text-white -translate-y-px"
										: "bg-transparent text-[#7c8399]"
								}`}
						...
						>
							{!isLoginMode && (
								<div className="flex flex-col gap-[0.3rem]">
									<label
										className="text-[0.78rem] text-[#454d62] tracking-[0.04em] uppercase"
										htmlFor="auth-name"
									>
										Name
									</label>
							...
							{authError && (
								<p
									className="text-[0.8rem] text-[#ff6b6b] bg-[rgba(255,107,107,0.08)] rounded-[10px] px-3 py-2"
									role="alert"
								>
									{authError}
								</p>
